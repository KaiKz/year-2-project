...
void BuildingUpdater::update(ParticleData *particles, float dt)
{
    size_t end_id = particles->alive_count;
    for (size_t i = 0; i < end_id; ++i)
    {
        
        if (particles->position.at(i).y < roof_level.y+roof_size.y/2&&particles->position.at(i).x > roof_level.x-100&&particles->position.at(i).z > roof_level.z-100)
            if (particles->position.at(i).y < roof_level.y+roof_size.y/2&&particles->position.at(i).x < roof_level.x+100&&particles->position.at(i).z < roof_level.z+100)

            particles->position.at(i).y = roof_level.y+5;
    }
}
